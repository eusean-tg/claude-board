use crate::db::{self, activity, projects as pq, tasks as tq};
...
pub fn start_planning(
    ...
    activity::add(
        ...
        None,
        "plan_started",
        &format!("Planning started: {}", topic.trim()),
        None,
    );
    ...
    std::thread::spawn(move || {
        ...
        let stderr = match child.stderr.take() {
            Some(s) => s,
            None => {
                log::error!("Planning: no stderr pipe");
                return;
            }
        };
        ...
        log::info!(
            "Planning: claude exited with code {}, full_text length: {}, tool_calls: {}, turns: {}",
            status,
            full_text.len(),
            tool_calls,
            turns
...
pub fn approve_plan(
    ...
    activity::add(
        ...
        None,
        "plan_completed",
        &format!("Plan approved: {} tasks created", created.len()),
        None,
    );
    Ok(created)
}
...
fn build_planning_prompt(
    ...
    format!(
        r#"You are a senior software architect performing a codebase analysis and task planning exercise. Your goal is to produce a precise, actionable task breakdown that another AI agent (Claude) can execute autonomously, one task at a time.
