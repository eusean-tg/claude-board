...
| [pnpm](https://pnpm.io) | Pinned in `package.json` | Package manager — `corepack enable pnpm` |
...
`pnpm run setup` installs both the root and `client/` dependency trees. `pnpm exec tauri dev` builds the Rust backend, starts the Vite dev server for the React frontend, and opens the app window with hot-reload enabled.
