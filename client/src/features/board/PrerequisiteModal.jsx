    ...
    >
      <ol className="space-y-2">
        {waves.map((wave, i) => (
          <li
            key={i}
            className="flex items-start gap-3 px-3 py-2 rounded-lg bg-surface-800/50 border border-surface-700/50"
          >
            ...
            <span className="min-w-0 flex-1">
              ...
              {i === lastIndex ? (
                <span className="block text-[10px] text-claude mt-0.5">{t('prerequisites.target')}</span>
              ) : wave.length > 1 ? (
                <span className="block text-[10px] text-surface-500 mt-0.5">{t('prerequisites.together')}</span>
              ) : null}
            </span>
          </li>
        ))}
      ...
      <div className="mt-4 flex justify-end gap-2">
        <button
          onClick={onClose}
          disabled={starting}
          className="px-3 py-1.5 text-xs text-surface-400 hover:text-surface-200 disabled:opacity-40"
        >
